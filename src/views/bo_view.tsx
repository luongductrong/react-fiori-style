...
export function BoView() {
  ...
  React.useEffect(() => {
    ...
    const stillVisible = selectedBoId ? filteredBizObjects.some((item) => item.BoId === selectedBoId) : false;
  ...
  const canUpdate = Boolean(selectedBo && editForm.BoType.trim() && editForm.BoTitle.trim() && editForm.Status.trim());
          ...
          <div className="flex flex-col gap-4 p-4">
            <Toolbar className="rounded-2xl border border-slate-200/80 bg-white/90 px-4 py-3 shadow-sm">
              ...
              <ToolbarButton
                ...
                onClick={() => navigate('/business-objects/create')}
              />
            </Toolbar>
          </div>
        </DynamicPageHeader>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4 p-2">
            <div className="flex flex-wrap items-start gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 text-sm text-slate-600">
                  BO ID {selectedBo.BoId} · Type {selectedBo.BoType}
                </div>
              </div>
              <div
                className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${getStatusTone(selectedBo.Status)}`}
              >
                {selectedBo.Status}
              </div>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-2xl bg-slate-50 p-4">
                ...
                <div className="mt-2 text-sm text-slate-600">
                  Created: {formatDateTime(selectedBo.Erdat, selectedBo.Erzet)}
                </div>
                <div className="mt-1 text-sm text-slate-600">Created By: {selectedBo.Ernam || '-'}</div>
                <div className="mt-1 text-sm text-slate-600">
                  Changed: {formatDateTime(selectedBo.Aedat, selectedBo.Aezet)}
                </div>
              ...
              <div className="rounded-2xl bg-slate-50 p-4">
                ...
                <div className="mt-2 text-sm text-slate-600">
                  Can link attachment: {selectedBo.__OperationControl?.link_attachment ? 'Yes' : 'No'}
                </div>
                <div className="mt-1 text-sm text-slate-600">
                  Editable: {selectedBo.__EntityControl?.Updatable ? 'Yes' : 'No'}
                </div>
                <div className="mt-1 text-sm text-slate-600">
                  Deletable: {selectedBo.__EntityControl?.Deletable ? 'Yes' : 'No'}
                </div>
            ...
            <List>
              ...
              <ListItemStandard
                text="Can link attachment"
                description={selectedBo.__OperationControl?.link_attachment ? 'Yes' : 'No'}
              />
