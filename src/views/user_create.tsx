	...
	return (
		...
		>
			<div className="mx-auto flex h-full w-full max-w-4xl flex-col gap-4 p-4">
				{errorMessage ? <MessageStrip design="Negative" hideCloseButton>{errorMessage}</MessageStrip> : null}
				...
				<div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
					<div className="grid gap-4 md:grid-cols-2">
						...
						<label className="flex flex-col gap-2">
							...
							<Select value={form.Role} onChange={(event) => setForm((prev) => ({ ...prev, Role: event.detail.selectedOption?.value || '' }))}>
								<Option value="">
									Select role
								</Option>
