...
import { AUTH_USER_LIST_FIELDS } from '@/features/auth-users/view-config';
import type { AuthUserListFieldId, AuthUserListFieldOption } from '@/features/auth-users/view-config';

function matchesSearch(field: AuthUserListFieldOption, search: string) {
...
interface UserViewSettingsProps {
  onSelectChange?: (selectedIds: AuthUserListFieldId[]) => void;
}

export function UserViewSettings({ onSelectChange }: UserViewSettingsProps) {
  const selectedIds = useViewStore((state) => state.authUserListVisibleFieldIds);
  const setSelectedIds = useViewStore((state) => state.setAuthUserListVisibleFieldIds);
  const [open, setOpen] = React.useState(false);
  const [search, setSearch] = React.useState('');
  const [showSelectedOnly, setShowSelectedOnly] = React.useState(false);
  const [draftSelectedIds, setDraftSelectedIds] = React.useState<AuthUserListFieldId[]>(selectedIds);

  const selectedFieldIds = React.useMemo(() => new Set(draftSelectedIds), [draftSelectedIds]);
  const visibleFields = React.useMemo(() => {
    return AUTH_USER_LIST_FIELDS.filter((field) => matchesSearch(field, search)).filter(
  ...
  const totalCount = AUTH_USER_LIST_FIELDS.length;
  ...
  const handleFieldToggle = function (fieldId: AuthUserListFieldId, checked: boolean) {
  ...
  const handleVisibleFieldsToggle = function (checked: boolean) {
    setDraftSelectedIds((prev) => {
      if (checked) {
        return AUTH_USER_LIST_FIELDS.filter(
